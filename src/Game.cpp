...
void Game::UpdatePlaying( const GameInput& gameInput )
{
	...
	// rotate
	if ( gameInput.bRotateClockwise )//( gameInput.bRotateClockwise )
	{
		TetrominoInstance testInstance = m_activeTetromino;
	...
	if ( gameInput.bRotateAnticlockwise )
	{
		...
		TetrominoInstance testInstance = m_activeTetromino;
		if (testInstance.m_rotation == 3)
		{
			testInstance.m_rotation = 0;			
		}
		else
		{
			++testInstance.m_rotation;			
...
void Game::Draw( Renderer& renderer )
{
	...
	{
	case kGameState_TitleScreen:
		renderer.DrawText("1. Начать игру", renderer.GetLogicalWidth() / 2 - 100, renderer.GetLogicalHeight() / 3, 0xffffffff);
		renderer.DrawText("2. Выйти", renderer.GetLogicalWidth() / 2 - 100, renderer.GetLogicalHeight() / 2, 0xffffffff);
		//renderer.DrawText( "Нажмите пробел для начала", renderer.GetLogicalWidth()/2 - 200, renderer.GetLogicalHeight()/2, 0xffffffff );
		break;
	case kGameState_Playing:
	...
	case kGameState_GameOver:
		DrawPlaying( renderer );
		renderer.DrawText( "ИГРА ОКОНЧЕНА", renderer.GetLogicalWidth() / 2 - 150, renderer.GetLogicalHeight() / 2, 0xffffffff );
		break;
    default:
        HP_FATAL_ERROR("Unhandled case");
	}
...
void Game::DrawPlaying( Renderer& renderer )
{
	...
	char text[128];
	snprintf( text, sizeof(text), "Линий: %u", m_numLinesCleared );
	...
	snprintf( text, sizeof(text), "Очки: %u", m_score );
	renderer.DrawText( text, 0, 180, 0xffffffff );
	snprintf( text, sizeof(text), "Рекорд: %u", m_hiScore );
